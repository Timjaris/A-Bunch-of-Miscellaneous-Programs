...
for i in range(n):
    ...
    for j in range(len(order)):
        print(dist)
        if len(order[j]) == 1:
            sen = letters.index(order[j])
            dist[sen] -= 1      
        if len(order[j]) == 2:
            sen1 = letters.index(order[j][0])
            sen2 = letters.index(order[j][1])
            dist[sen1] -= 1
            dist[sen2] -= 1
        #The test:
        if max(dist)*2 > sum(dist):
            party = letters[dist.index(max(dist))]
            print("Party B has " + str(dist[1]) + " out of " + str(sum(dist)) + "votes")
            WrongAnswerExit(p, i, "You failed! Party {} has used the crisis, and now controls the country".format(party))
