...
while again == 'y' or again == 'Y': #float(x)/float(len(stocklist)) < 1: #
    try:
        ...
        if len(options) < 180000:
            ...
            o = -(20.0/199.0)*PE_to_Industry*100+float(2010.0/199.0)
            if o>0:
                plus = '+'
            else:
                plus = ''
            ...
            p = (1.0/6.0)*por*100.0 - 5
            if p>0:
                plus = '+'
            else:
                plus = ''
            print "Pay out Ratio = ", '%23s' %('{0:.0%}'.format(por)), '%10s' %('('+plus+'{0:.2f}'.format(p)+')')
